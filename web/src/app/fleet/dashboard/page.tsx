...
type Search = { monthOffset?: string };

async function loadDashboard(monthOffset?: number): Promise<FleetDashboardSnapshot | null> {
  const q =
    monthOffset != null && monthOffset !== 0 ? `?monthOffset=${monthOffset}` : "";
  const res = await fleetServerFetch(`/fleet/dashboard${q}`);
  if (!res) return null;
...
export default async function FleetDashboardPage({ searchParams }: PageProps) {
  ...
  const raw = sp.monthOffset != null && sp.monthOffset !== "" ? Number(sp.monthOffset) : 0;
  const monthOffset = Number.isFinite(raw) ? Math.min(0, Math.max(-11, Math.trunc(raw))) : 0;
  const data = await loadDashboard(monthOffset);
  const writeTickets = canWriteTickets(auth);
    ...
    <FleetPageMain>
      <div className="mb-8">
        ...
        <p className="mt-3 max-w-2xl text-sm text-zinc-400">
          Rezumat flotă — trend costuri/curse pe perioadă, indicatori, acțiuni rapide și liste de atenție.
        </p>
        {data ? (
      ...
      {data ? (
        <FleetDashboardView data={data} canWriteTickets={writeTickets} />
      ) : (
        <div className="rounded-xl border border-amber-900/50 bg-amber-950/30 p-6 text-sm text-amber-100">
          ...
          <Link href="/fleet/dashboard" className="mt-4 inline-block text-sky-400 hover:text-sky-300">
            Reîncarcă
          </Link>
        </div>
      )}
