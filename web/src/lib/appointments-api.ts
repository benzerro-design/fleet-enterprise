...
export type CalendarAppointment = {
  ...
  fleetCounterProposedBy?: "manager" | "driver" | "supplier" | "admin" | null;
...
export function appointmentFleetCanCounterPropose(
  ...
  if (!appointmentHasSlot(a.scheduledAt)) return false;
  if (appointmentIsFullyValidated(a)) return false;
  if (opts?.negotiate) return true;
  return !a.managerConfirmedAt && !a.driverAcknowledgedAt;
}

export function appointmentFleetCanRepropose(
  a: {
    status: string;
    scheduledAt?: string | null;
    managerConfirmedAt?: string | null;
  ...
  return appointmentFleetCanCounterPropose(a, opts);
}
