...
export const TICKET_GRID_COLUMNS: TicketGridColumnDef[] = [
  ...
  { key: "priority", label: "Prio", defaultVisible: true, canHide: true, minWidth: 96, defaultWidth: 108, maxWidth: 160 },
  ...
  { key: "driver", label: "Șofer", defaultVisible: true, canHide: true, minWidth: 100, defaultWidth: 120, maxWidth: 200 },
  { key: "routing", label: "Nivel", defaultVisible: true, canHide: true, minWidth: 40, defaultWidth: 44, maxWidth: 72 },
  { key: "owner", label: "Owner", defaultVisible: true, canHide: true, minWidth: 88, defaultWidth: 100, maxWidth: 180 },
...
export const TICKET_GRID_STORAGE_KEY = "fleet-ticket-grid-columns-v2";
...
export function readTicketGridLayout(): TicketGridLayout {
  ...
  try {
    ...
    if (!raw) {
      // Migrează ordine/hidden din v1 dacă există.
      const legacy = localStorage.getItem("fleet-ticket-grid-columns-v1");
      if (legacy) {
