...
type Props = { entryId: string; label?: string; redirectTo?: string };

export function DeleteMaintenanceButton({ entryId, label, redirectTo }: Props) {
  const router = useRouter();
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function onDelete() {
    const ok = window.confirm(`Ștergi înregistrarea "${label ?? entryId}"?`);
    if (!ok) return;
    setPending(true);
    setError(null);
    try {
      const res = await fetch(`/api/maintenance/${entryId}`, { method: "DELETE" });
