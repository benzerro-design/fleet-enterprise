...
import { clientsBrowserBase, fleetJsonHeaders, type ClientDocumentRecord } from "@/lib/clients-api";
import {
  CLIENT_DOCUMENT_TYPE_OPTIONS,
  clientDocumentTypeLabel,
...
export function ClientDocumentsPanel({ clientId, initialDocuments }: Props) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);
  const [documents, setDocuments] = useState(initialDocuments);
  const [documentTypeCode, setDocumentTypeCode] = useState("cui");
  ...
  const base = `${clientsBrowserBase}/${clientId}/documents`;

  async function refresh() {
    const res = await fetch(base);
    if (res.ok) {
      setDocuments((await res.json()) as ClientDocumentRecord[]);
    }
    router.refresh();
  }
  ...
  return (
    <section className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-5">
      <h2 className="text-lg font-medium text-zinc-100">Documente client</h2>
      <p className="mt-1 text-sm text-zinc-500">CUI, autorizații, contracte și alte documente ale organizației.</p>
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-zinc-500">
                  {clientDocumentTypeLabel(d.documentTypeCode)}
              ...
              <button
                ...
                disabled={pending}
                className="shrink-0 text-rose-400 hover:underline disabled:opacity-60"
              >
                Șterge
              </button>
            </li>
      ...
      <div className="mt-6 border-t border-zinc-800 pt-5">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <div>
            ...
            >
              {CLIENT_DOCUMENT_TYPE_OPTIONS.map((o) => (
