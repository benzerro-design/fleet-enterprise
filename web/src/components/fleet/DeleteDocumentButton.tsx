...
type Props = { documentId: string; label?: string; redirectTo?: string };

export function DeleteDocumentButton({ documentId, label, redirectTo }: Props) {
  const router = useRouter();
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function onDelete() {
    const ok = window.confirm(`Ștergi documentul "${label ?? documentId}"?`);
    if (!ok) return;
    setPending(true);
    setError(null);
    try {
